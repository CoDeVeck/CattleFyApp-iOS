...
class ReporteEngordeViewModel {
    ...
    private var datosGrafico: [ReporteGrafico1] = []
    
    // MARK: - Callbacks
    var onLotesUpdated: (() -> Void)?
    var onCategoriasUpdated: (() -> Void)?
    var onReporteUpdated: ((ReporteProduccionEngordeDTO) -> Void)?
    var onGraficoUpdated: (([ReporteGrafico1]) -> Void)?
    ...
    func aplicarFiltros(
      ...
      // Fetch datos del reporte (estadísticas de engorde)
      fetchReporteEngorde(filtros: filtros)
      
      // Fetch datos del gráfico (evolución de peso)
      fetchGraficoEngorde(filtros: filtros)
    }
    
    // MARK: - Fetch Reporte Engorde
    private func fetchReporteEngorde(filtros: ReporteProduccionFiltros) {
      isLoading?(true)
      
      service.fetchReporteEngorde(granjaId: granjaId, filtros: filtros) { [weak self] result in
        DispatchQueue.main.async {
          self?.isLoading?(false)
    ...
    // MARK: - Fetch Gráfico Engorde
    private func fetchGraficoEngorde(filtros: ReporteProduccionFiltros) {
      isLoading?(true)
      
      service.fetchGraficoEngorde(granjaId: granjaId, filtros: filtros) { [weak self] result in
        DispatchQueue.main.async {
          self?.isLoading?(false)
